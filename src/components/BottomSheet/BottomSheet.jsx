...
class BottomSheet extends Component {
  ...
  togglePanel = () => {
    ...
    const { animationDuration, animation, sliderMinHeight, onOpen, onClose } = this.props;

    Animated.timing(this.panelHeightValue, {
      duration: animationDuration,
      easing: animation,
      toValue: this.panelHeightValue._value === 0 ? contentHeight - sliderMinHeight : 0,
  ...
  _setPanResponders() {
    ...
    this._childPanResponder = PanResponder.create({
      ...
      onPanResponderRelease: (_, gestureState) => gestureState.dy > 0 && this.togglePanel(),
    });
  }
    ...
    return (
      <Animated.View
        ...
        style={{
          ...
          transform: [{ translateY: this.panelHeightValue }, { scale: isPanelVisible ? 1 : 0 }],
        }}
      >
        <View
        ...
        >
          ...
          <View style={[styles.innerContent, innerContentStyle]}>
            {typeof children === 'function' ? children(this._handleScrollEndDrag) : children}
          </View>
        </View>
      </Animated.View>
    );
...
BottomSheet.propTypes = {
  ...
  sliderMinHeight: (props, propName, _) => {
    if (props[propName] > props.sliderMaxHeight) {
      return new Error('sliderMinHeight value cannot be greater than sliderMaxHeight');
    }
  },
